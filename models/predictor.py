...
class Predictor(nn.Module):
    ...
    def forward(self, initial_state, latent_variable, num_steps):
        ...
        # latent_variable shape n l latent
        # predictor shape n l c c

        next_state_ground_true = initial_state[:, num_steps:]

        n, l, c = initial_state.shape
        predictor = self.predictor_dictionary(latent_variable).view(n, l, c, c)

        # predictions = []
        next_state = initial_state
        for i in range(num_steps):
            next_state = torch.einsum("nlc,nlcb->nlb", next_state[:, :-num_steps], predictor)
